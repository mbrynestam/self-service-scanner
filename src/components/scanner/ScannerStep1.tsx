...
interface ScannerStep1Props {
  onSubmit: (url: string, honeypot?: string) => void;
}
...
export default function ScannerStep1({ onSubmit }: ScannerStep1Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Bot detection on client side: honeypot field should be empty
    // (Backend will also validate this, but we can skip the request entirely)
    if (honeypot) {
    ...
    try {
      ...
      // Send honeypot value to backend (should be empty for real users)
      onSubmit(validUrl, honeypot);
    } catch {
